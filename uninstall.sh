...
sed -i 's/.*# installed by config-pa-de-solminihac/d' ~/.bashrc
sed -i 's/source ~\/.bashrc_common/d' >> ~/.bashrc
rm -f ~/.bashrc_common
sed -i 's/.*# installed by config-pa-de-solminihac/d' ~/.vimrc
sed -i 's/source ~\/.vimrc_common/d' >> ~/.vimrc
rm -f ~/.vimrc_common
